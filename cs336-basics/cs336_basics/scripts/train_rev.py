...
def rev_get_batch(*args, **kwargs):
    batch_x, batch_y = get_batch(*args, **kwargs)
    rev_batch_x = batch_y.flip(-1)
    rev_batch_y = batch_x.flip(-1)
    return rev_batch_x, rev_batch_y
    ...
    for i in tqdm(range(train_steps)):
        ...
        for micro_step_idx in range(gradient_accumulation_steps):
            ...
            with amp_ctx:
                ...
                next_batch_x, next_batch_y = rev_get_batch(
                    train_data,
                    batch_size=batch_size,
                    context_length=context_length,
                    device=device,
